...
public:
	...
	{
		m_index = st.m_index;
		//m_p = st.m_p;// дл€  встроеного массива move&&  смысла не имеет, птимизации нет, по факту получитс€ MyStack(const MyStack& st)
		std::swap(m_p, st.m_p);
		st.m_index = 0;
		std::cout << "\nrun constr move&& ";
	...
	{// дл€  встроеного массива oper move&&  смысла не имеет, нетоптимизц. если кл. MyString корректно спроектрован
		if (this == &st)
		{
			return *this;
		}
		...
		m_index = st.m_index;
		//m_p = st.m_p; // дл€ st.m_p деструктор отработает по m_n
		std::swap(m_p, st.m_p);
		st.m_index = 0;
	...
	{	//сколько сконструировал пользователь и по T(), столько и удал€ю => m_n
		/*for (size_t i = 0; i < m_n; i++)
			m_p[i].~T();*/ //Ћишн€€ работа, деструктор и так  будет вызван компил_м
	}
