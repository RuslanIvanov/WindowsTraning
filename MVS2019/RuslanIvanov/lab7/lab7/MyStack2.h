...
public:
	...
	MyStack2& operator=(const MyStack2& st)
	{
		...
		while (p)//цикл по большей послед
		{
			...
			{//end copy
				...
				{
					pop();//удал€ет не то, надо что б точна€ копи€ была
					//pcopy = pcopy->m_next;
				/*	if (pcopy != nullptr)
					{
	...
	/*
	MyStack2.h: In member function СT& MyStack2<T, <anonymous> >::operator[](size_t) [with T = int; unsigned int m_n = 4u; size_t = unsigned int]Т:
	...
	*/
	...
	void push(const T& p) // встав€лть
	{
		if (m_index < m_n)//голова - это последний который вставили
		{
			...
			{ std::cout << e.what(); throw; } //перебрасывает объект исключени€ типа std :: length_error;

			if (m_head)
			{
				pNode->m_next = m_head;// вставка перед головой
				m_head = pNode;// текущий становитс€ головой
			}
			else { m_head = pNode; }
