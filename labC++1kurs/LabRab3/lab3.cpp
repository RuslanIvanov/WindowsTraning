...
{ 
 ...
 //pn=0;
    ...
    for(int i=0; i<N; i++)  
	 {//for1
	  for(int j =0;j<M-1;j++)// n-1, так как последний автоматически окажется на своем месте
	   {//for2
		int max = j; 
		for(int k=j+1; k<M; k++)
		 {
...
*/
	...
	//двухмерного массива
    double *p1=new double[N]; //одномерный масс N разм
	int j=0;
	int Sum1=0;
    int **ppmas = new int*[N];
	for(int i=0;i<N;i++)
...
/**/
	...
	const int nNstr =sizeof(dArray[0][0])/sizeof(dArray[0][0][0]);/// колличество эл в строке
